...
const Post: React.FC<{postId:string, username:string,initialLikeStatus:boolean,initialBookmarkStatus:boolean,onLikeToggle:(postId:string, initialLikeStatus:boolean) => void, onBookmarkToggle:(postId:string, initialBookmarkStatus:boolean) =>void, timestamp:string,content:string,comments:{ id: number,
  user: string,
  text: string,}[]}> = ({ postId, username, initialLikeStatus, initialBookmarkStatus, onLikeToggle, onBookmarkToggle, timestamp,content,comments }) => {
...
function convertDateToMinutes(date:string) {
  ...
  const dateInMilliseconds = dateObj.getTime();
